...
- `Spreadsheet::sheets_for_drawable_object(id)` resolves one raw 5020-5030 cluster object id back to the owning sheet(s).
...
- `Spreadsheet::decoded_tables()` is the authoritative table view: it follows each model's tiles and scoped DataLists, merges multi-tile row ranges, and avoids cross-table string-key collisions. `tables_for_sheet(sheet)` lifts the same decode to one sheet's tables in sheet order.
- `Spreadsheet::formula_records()` decodes type-4008 formula records from `Index/CalculationEngine.iwa`; field 1 is exposed as a raw `FormulaRecordKey`, field 2 matches formula ids stored on some formula-result cells, `formula_record_for_cell(cell)` resolves cached formula cells when a matching record exists, `formula_records_for_table(table)` / `formula_records_for_model(model)` / `formula_records_for_sheet(sheet)` resolve formula records referenced by decoded tables, field 6 is exposed as a structural `FormulaExpression`, field 6.5 is retained as raw expression bytes, fields 7/8 are exposed as raw `FormulaBoundsPair` values, raw fields 13/14/15 are retained as opaque bytes, fields 13/14/15 can also be re-decoded as nested `ProtoMessage`s when their payloads use the currently observed protobuf shape, fields 13 and 15 additionally expose decoded repeated raw varint lists when their payloads use the currently observed nested shape, and referenced type-4009 auxiliary records can be resolved at record/table/model/sheet scope.
...
- `Spreadsheet::header_storage_bucket()` decodes type-6006 `HeaderStorageBucket` archives structurally, and `table_header_storage(model)` resolves one table's row-indexed and column-indexed buckets as a pair. Each table model references a row-indexed bucket via `DataStore.field 1.2` and a column-indexed bucket via `DataStore.field 2`; entry field 2 is exposed as raw fixed32 bits plus an f32 reinterpretation, while entry fields 3-4 are still raw structural fields.
...
- Formula expressions/dependency graph are not decoded; formula result cells preserve the formula-result marker, formula id, and cached value through `CellValue::Formula`, and some formula ids resolve to type-4008 `FormulaRecord` objects. FormulaRecord field 6 is retained as a structural wrapper, field 6.5 is retained as raw expression bytes, and fields 7/8 plus type-4009 auxiliary records are decoded structurally, but their exact graph semantics are not named yet.
- Header storage bucket axis roles are decoded, and entry field 2 can be reinterpreted as finite nonnegative f32 across current fixtures, but layout semantics for entry fields 2-4 are not fully cross-validated.
