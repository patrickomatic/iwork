...
- `Spreadsheet::header_storage_bucket()` decodes type-6006 `HeaderStorageBucket` archives structurally, and `table_header_storage(model)` resolves one table's row-indexed and column-indexed buckets as a pair. Each table model references a row-indexed bucket via `DataStore.field 1.2` and a column-indexed bucket via `DataStore.field 2`; entry fields 2-4 are still raw structural fields.
